...
from utils.reconstruct_mdm import build_molecule
...
class QVinaDockingTask(BaseDockingTask):
    ...
    @classmethod
    def from_generated_data(cls, protein_filename, mol, protein_root='./data/crossdocked_pocket10', **kwargs):
        # protein_fn = os.path.join(
        #     os.path.dirname(data.ligand_filename),
        #     os.path.basename(data.ligand_filename)[:10] + '.pdb'
        # )
        protein_fn = os.path.join(
            os.path.dirname(protein_filename),
            os.path.basename(protein_filename)
        )
    ...
    def __init__(self, pdb_block, ligand_rdmol, conda_env='adt', tmp_dir='./tmp', use_uff=False, center=None):
    ...
    def get_results(self):
        if self.proc is None:   # Not started
            ...
            if self.output is None:
                ...
                error = self.proc.stderr.readlines()
                # print(self.output)
                print(error)
