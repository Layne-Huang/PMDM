...
class GaussianDiffusion(nn.Module):
    def __init__(self, net, betas):
        ...
        self.register("sqrt_recip_alphas_cumprod", torch.rsqrt(alphas_cumprod))  # 1/sqrt(alpha_bar)
    ...
    def p_loss(self, x_0, t, context, noise=None):
        if noise is None:
            noise = torch.randn_like(x_0)

        x_noise = self.q_sample(x_0, t, noise)
        x_recon = self.net(x_noise, context)
